...
.DESCRIPTION
Este script crea el arbol completo de carpetas segun la estructura VIDA DIGITAL definida en VIDA_DIGITAL_Estructura.md.
Soporta modo simulacion (-Simular) para revisar antes de crear.

.PARAMETER RutaRaiz
Ruta raiz donde se creara la estructura VIDA DIGITAL (ej. D:\OneDrive\VIDA DIGITAL)

.PARAMETER Simular
Si esta presente, muestra que carpetas se crearian sin crearlas realmente
...
Fecha: 2026-02-12
Versión: 1.2 (ASCII Only strings)
#>
...
# Funcion para crear carpeta (o simular)
function New-Folder-Safe {
    param([string]$Path)
    
    if ($Simular) {
        Write-Host ("[SIMULACION] Crearia: " + $Path) -ForegroundColor Yellow
    }
    else {
...
Write-Host "  CREACION ESTRUCTURA VIDA DIGITAL" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""

if ($Simular) {
    Write-Host "MODO SIMULACION: No se creara ninguna carpeta" -ForegroundColor Yellow
    Write-Host ""
}

Write-Host ("Ruta raiz: " + $RutaRaiz) -ForegroundColor White
Write-Host ""
...
if ($Simular) {
    Write-Host "MODO SIMULACION: No se creo ninguna carpeta." -ForegroundColor Yellow
}
else {
